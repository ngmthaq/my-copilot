---
name: vuejs-options-api-unit-test
description: "Vue 3 Options API unit testing — Vitest + Vue Testing Library: rendering components, querying elements, simulating user events, testing async behavior, mocking services, and Storybook for component documentation. Use when: writing unit or integration tests for Vue 3 Options API components; writing stories for component documentation."
---

# Vue 3 Options API Unit Testing Skill

## Overview

This skill covers testing Vue 3 Options API components with **Vitest** + **Vue Testing Library (VTL)** and documenting components with **Storybook**. VTL encourages testing the way users actually interact with the UI, not implementation details. Testing patterns are nearly identical to Composition API — the main difference is what you mock (services instead of composables).
...
Components that use Pinia stores, Vue Router, or Vuetify need wrappers in tests.
...
In Options API components, API calls happen in service classes. Mock the service module.
...
// Mock an auth service
vi.mock("@/services/authService", () => ({
  authService: {
    login: vi.fn().mockResolvedValue({ token: "mock-token", userId: "1" }),
  },
}));
```
...
it("calls getUsers on created", async () => {
...
When a component uses a mixin that has side effects, override it via `global.mixins`:

```typescript
it("renders without mixin side effects", () => {
  renderWithProviders(UsersPage, {
    global: {
      mixins: [
        {
          // Override the mixin's created hook
          created() {
            // no-op
          },
        },
...
Or mock the mixin module directly:

```typescript
vi.mock("@/mixins/paginationMixin", () => ({
  paginationMixin: {
    data() {
      return { page: 1, pageSize: 10, totalItems: 0 };
    },
    computed: {
      totalPages() {
        return 1;
...
it("submits the form with valid data", async () => {
  ...
  vi.mock("@/services/authService", () => ({
    authService: {
      login: vi.fn().mockResolvedValue({ token: "t", userId: "1" }),
    },
  }));
